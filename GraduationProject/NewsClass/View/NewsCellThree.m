...
@property(nonatomic,strong)WBBaseImageView* faceImage2;
//文章标题
...
-(void)setModelSource:(NewsModel *)modelSource{
    ...
    if (modelSource.thumbnail_pic_s03) {
        [self.faceImage2 setImageWithUrl:modelSource.thumbnail_pic_s03];
    }
    
    if (modelSource.title) {
        self.title.text = modelSource.title;
        self.title.textLayout = [NewsModel textLayout:modelSource.title size:CGSizeMake(SCREENWIDTH - 20, 40)];
    }
...
-(void)layoutSubviews{
    ...
    [self.faceImage2 mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(100, 80));
        make.leading.equalTo(self.faceImage1.mas_trailing).with.offset(10);
        make.top.equalTo(self.title.mas_bottom).with.offset(30);
    }];
}
...
-(WBBaseImageView *)faceImage2{
    if (!_faceImage2) {
        _faceImage2 = [[WBBaseImageView alloc] init];
        [self.contentView addSubview:_faceImage2];
    }
    return _faceImage2;
}
