...
- (instancetype)initWithFrame:(CGRect)frame datas:(NSArray *)array dict:(NSDictionary *)dict{
...
- (void)setupUI {
    ...
    switch ([self.dict[@"status"] intValue]) {
        case 1:
            self.header.type.text= @"在途中";
            break;
        case 2:
            self.header.type.text= @"派件中";
            break;
        case 3:
            self.header.type.text= @"已签收";
            break;
        case 4:
            self.header.type.text= @"派送失败";
            break;
        default:
            break;
    }
    ...
    self.header.numLabel.text = self.dict[@"number"];
